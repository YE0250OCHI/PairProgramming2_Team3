...
- テストが未実施（モンキーテストのみ）
- ロギングの設置はできているが、ファイル出力の設定ができていない
- 状態遷移がうまくいかない（タクシー状態変更→JOB状態の更新とならない）
- スタブおよびスタブ側APIの動作確認がまだ
...
- JOBとタクシーの２要素での状態遷移とその整合性
- 各状態に応じて提供するデータの更新、それを受け取った画面側でのふるまい変更
- ダッシュボード機能による見える化
- JSによるインタラクティブな画面
- スタブを用いた、2デバイスでの状態遷移と排他制御
...
|WARN|400系エラー|[失敗]タクシー割当を拒否|
|ERROR|異常操作|[エラー]JOB一覧取得:SQL_ERROR|
